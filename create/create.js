...
async function onEncrypt() {
  ...
  const encrypted = await generateFragment(url, password, hint, useRandomSalt,
      useRandomIv);
  const output = `https://johnadik.github.io/link-lock/#${encrypted}`;

  document.querySelector("#output").value = output;
  highlight("output");

  // Adjust "Hidden Bookmark" link
  document.querySelector("#bookmark").href = `https://johnadik.github.io/link-lock/hidden/#${encrypted}`;
...
function myFunction() {
  var x = document.getElementById("#password #confirm-password");
  if (x.type === "password") {
    x.type = "text";
  } else {
    x.type = "password";
